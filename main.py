...
# 2. NATR (Current Volatility Percentage)
df['NATR'] = talib.NATR(high_arr, low_arr, close_arr, timeperiod=14)

# 3. Base NATR (100-Hour Rolling Average of NATR, shifted to ignore current candle)
df['Base_NATR'] = df['NATR'].shift(2).rolling(window=100).mean()

# 4. Grid Spacing Simulation Logic
...
st.markdown("### 1. Market Price & Absolute Range Values")
st.caption("How TR (Individual Candle Range) and ATR (Averaged Smooth Range) track the asset price.")
# We display Close on left axis, TR/ATR on right via a clear chart selection
st.line_chart(df[['close', 'ATR', 'TR']], y_label="Price / Volatility In USD")
...
st.line_chart(df[['NATR', 'Base_NATR']], y_label="Percentage Value (%)")
...
st.line_chart(df['Dynamic_Spacing'], y_label="Calculated Grid Spacing (%)")
